...
export default function CastModal({ casts = [], visible, onClose, onRemoveClick }) {
    return (
        <ModalContainer ignoreContainer onClose={onClose} visible={visible}>
            <div className="space-y-2 dark:bg-primary bg-white rounded max-w-[45rem] 
            max-h-[40rem] overflow-auto p-2 custom-scroll-bar">
                {casts.map(({ profile, roleAs, leadActor }) => {
                    ...
                    return <div key={id} className="flex space-x-3 dark:bg-secondary bg-white drop-shadow-md rounded">
                        <img className='w-16 h-16 rounded object-cover aspect-square'
                            src={avatar} alt={name} />
                        <div className="w-full flex flex-col justify-between">
                            ...
                            {leadActor && <AiOutlineCheck className='text-light-subtle dark:text-dark-subtle'></AiOutlineCheck>}
                        </div>
                        <button onClick={() => onRemoveClick(id)} className='dark:text-white text-primary 
                    hover:opacity-80 transition p-2'><AiOutlineClose /></button>
                    </div>
